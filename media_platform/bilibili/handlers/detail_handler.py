...
class DetailHandler(BaseHandler):
    ...
    async def get_specified_videos(self):
        """
        Get the information and comments of the specified videos
        Returns:
            None
        """
        utils.logger.info(
            "[DetailHandler.get_specified_videos] Begin get bilibili specified videos"
        )

        checkpoint = Checkpoint(
            platform=constant.BILIBILI_PLATFORM_NAME, mode=constant.CRALER_TYPE_DETAIL
        )
        ...
        # 使用指定的视频ID列表
        bvids_list = config.BILI_SPECIFIED_ID_LIST
        video_infos: List[VideoIdInfo] = await self.video_processor.batch_get_video_list_from_bvids(
            bvids_list, checkpoint_id=checkpoint.id
        )
