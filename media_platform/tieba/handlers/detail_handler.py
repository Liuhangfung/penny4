...
class DetailHandler(BaseHandler):
    ...
    async def get_specified_notes(
        self, note_id_list: List[str] = None
    ):
        """
        Get the information and comments of the specified post
        Args:
            note_id_list: List of note IDs to crawl, defaults to config.TIEBA_SPECIFIED_ID_LIST

        Returns:
            None
        """
        utils.logger.info(
            "[DetailHandler.get_specified_notes] Begin get tieba specified notes"
        )

        if note_id_list is None:
            note_id_list = config.TIEBA_SPECIFIED_ID_LIST

        checkpoint = Checkpoint(platform=constant.TIEBA_PLATFORM_NAME, mode=constant.CRALER_TYPE_DETAIL)
